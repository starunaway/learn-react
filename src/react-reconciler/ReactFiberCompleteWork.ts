...
import { popCacheProvider, Cache } from './ReactFiberCacheComponent';
...
let appendAllChildren: any;
let updateHostContainer: any;
let updateHostComponent: any;
let updateHostText: any;
if (supportsMutation) {
  // Mutation mode
...
function completeWork(
  ...
  switch (workInProgress.tag) {
    ...
    case WorkTag.ClassComponent: {
      console.error('complete  ClassComponent 逻辑未实现');
    ...
    case WorkTag.HostRoot: {
      ...
      if (enableCache) {
        ...
        if (cache !== previousCache) {
          ...
          workInProgress.flags |= Flags.Passive;
        }
        ...
        if (wasHydrated) {
          ...
          if (current !== null) {
            ...
            if (
              ...
              (workInProgress.flags & Flags.ForceClientRender) !== Flags.NoFlags
            ) {
              ...
              // be empty).
              workInProgress.flags |= Flags.Snapshot;
      ...
      if (enableTransitionTracing) {
        if ((workInProgress.subtreeFlags & Flags.Visibility) !== Flags.NoFlags) {
    ...
    case WorkTag.HostComponent: {
      ...
      if (current !== null && workInProgress.stateNode != null) {
        ...
        const wasHydrated = popHydrationState(workInProgress);
        if (wasHydrated) {
          console.error('SSR 相关,这里应该是进不来的.wasHydrated=', wasHydrated);
    ...
    case WorkTag.SuspenseComponent: {
      console.error('complete  SuspenseComponent 逻辑未实现');
      ...
      //     if (workInProgress.flags & Flags.ShouldCapture) {
      ...
      // if ((workInProgress.flags & Flags.DidCapture) !== Flags.NoFlags) {
      //   // Something suspended. Re-render with the fallback children.
      //   workInProgress.lanes = renderLanes;
      //   // Do not reset the effect list.
      //   if (
      //     enableProfilerTimer &&
      //     (workInProgress.mode & TypeOfMode.ProfileMode) !== TypeOfMode.NoMode
      //   ) {
      ...
      //   if (cache !== previousCache) {
      //     // Run passive effects to retain/release the cache.
      //     offscreenFiber.flags |= Flags.Passive;
      //   }
      // }
      ...
      //   if (enableTransitionTracing) {
      //     const offscreenFiber: Fiber = (workInProgress.child!);
      //     offscreenFiber.flags |= Flags.Passive;
      //   }
      ...
      //   if (nextDidTimeout) {
      //     const offscreenFiber: Fiber = (workInProgress.child!);
      //     offscreenFiber.flags |= Flags.Visibility;
    ...
    case WorkTag.HostPortal:
      console.error('complete  HostPortal 逻辑未实现');
    ...
    case WorkTag.IncompleteClassComponent: {
      console.error('complete  IncompleteClassComponent 逻辑未实现');
    ...
    case WorkTag.SuspenseListComponent: {
      console.error('complete  SuspenseListComponent 逻辑未实现');
    ...
    case WorkTag.ScopeComponent: {
      // read: 源码里特性不支持，直接break
      break;
    }
    case WorkTag.OffscreenComponent:
    case WorkTag.LegacyHiddenComponent: {
      console.error('complete  LegacyHiddenComponent or OffscreenComponent 逻辑未实现');
