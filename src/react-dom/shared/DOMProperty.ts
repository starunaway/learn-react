...
class PropertyInfoRecord {
  ...
  constructor(
    ...
    this.acceptsBooleans =
      type === PropertyType.BOOLEANISH_STRING ||
      type === PropertyType.BOOLEAN ||
      type === PropertyType.OVERLOADED_BOOLEAN;
...
export function shouldIgnoreAttribute(
  ...
  if (propertyInfo !== null) {
    return propertyInfo.type === PropertyType.RESERVED;
  }
...
export function shouldRemoveAttributeWithWarning(
  ...
  if (propertyInfo !== null && propertyInfo.type === PropertyType.RESERVED) {
    return false;
  }
  switch (typeof value) {
    case 'function':
  ...
  if (propertyInfo !== null) {
    switch (propertyInfo.type) {
      case PropertyType.BOOLEAN:
        return !value;
      case PropertyType.OVERLOADED_BOOLEAN:
        return value === false;
      case PropertyType.NUMERIC:
        return isNaN(value);
      case PropertyType.POSITIVE_NUMERIC:
        return isNaN(value) || (value as number) < 1;
    }
  }
  return false;
}
...
['contentEditable', 'draggable', 'spellCheck', 'value'].forEach((name) => {
  properties[name] = new PropertyInfoRecord(
    name,
    PropertyType.BOOLEANISH_STRING,
    false, // mustUseProperty
