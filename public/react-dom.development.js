(function (global, factory) {
  ...
  var STYLE = 'style';
  var HTML$1 = '__html'; // HTML parsing normalizes CR and CRLF to LF.
  ...
  function setInitialProperties(domElement, tag, rawProps, rootContainerElement) {
    var isCustomComponentTag = isCustomComponent(tag, rawProps); // TODO: Make sure that we check isMounted before firing any of these events.
  ...
  function diffHydratedProperties(domElement, tag, rawProps, parentNamespace, rootContainerElement, isConcurrentMode) {
  ...
  function createInstance(type, props, rootContainerInstance, hostContext, internalInstanceHandle) {
    var domElement = createElement(type, props, rootContainerInstance, hostContext);
    precacheFiberNode(internalInstanceHandle, domElement);
    updateFiberProps(domElement, props);
    return domElement;
  }
  function appendInitialChild(parentInstance, child) {
    console.log("真实的 dom 节点 添加子元素");
  ...
  function hydrateInstance(instance, type, props, rootContainerInstance, hostContext, internalInstanceHandle) {
    ...
    var parentNamespace;
    parentNamespace = hostContext; // TODO: Temporary hack to check if we're in a concurrent root. We can delete
    // when the legacy root API is removed.

    var isConcurrentMode = (internalInstanceHandle.mode & ConcurrentMode) !== NoMode;
    return diffHydratedProperties(instance, type, props, parentNamespace, rootContainerInstance, isConcurrentMode);
  }
  ...
  var emptyContextObject = {}; // A cursor to the current merged context object on the stack.
  ...
  function applyDerivedStateFromProps(workInProgress, ctor, getDerivedStateFromProps, nextProps) {
    ...
    var partialState = getDerivedStateFromProps(nextProps, prevState); // Merge the partial state and the previous state.
  ...
  function ChildReconciler(shouldTrackSideEffects) {
    ...
    function updateElement(returnFiber, current, element, lanes) {
      ...
      if (current !== null) {
        if (current.elementType === elementType || // Lazy types should reconcile their resolved type.
      ...
      while (child !== null) {
        ...
        if (child.key === key) {
          ...
          if (elementType === REACT_FRAGMENT_TYPE) {
            ...
            if (child.elementType === elementType || // Lazy types should reconcile their resolved type.
            // We need to do this after the Hot Reloading check above,
            // because hot reloading has different semantics than prod because
            // it doesn't resuspend. So we can't let the call below suspend.
  ...
  function updateMemoComponent(current, workInProgress, Component, nextProps, renderLanes) {
    if (current === null) {
      ...
      Component.defaultProps === undefined) {
        var resolvedType = type; // If this is a plain function component without default props,
  ...
  function mountLazyComponent(_current, workInProgress, elementType, renderLanes) {
    ...
    var hint = ''; // This message intentionally doesn't mention ForwardRef or MemoComponent
  ...
  function mountIndeterminateComponent(_current, workInProgress, Component, renderLanes) {
    ...
    hasId = checkDidRenderIdHook(); // React DevTools reads this flag.
  ...
  function updateSuspenseFallbackChildren(current, workInProgress, primaryChildren, fallbackChildren, renderLanes) {
    ...
    workInProgress.child !== currentPrimaryChildFragment) {
      ...
      primaryChildFragment = progressedPrimaryFragment;
      primaryChildFragment.childLanes = NoLanes;
      primaryChildFragment.pendingProps = primaryChildProps; // The fallback fiber was added as a deletion during the first pass.
  ...
  function updateContextConsumer(current, workInProgress, renderLanes) {
    ...
    var newChildren;
    newChildren = render(newValue); // React DevTools reads this flag.
  ...
  function bubbleProperties(completedWork) {
    ...
    if (!didBailout) {
      ...
      while (_child !== null) {
        newChildLanes = mergeLanes(newChildLanes, mergeLanes(_child.lanes, _child.childLanes)); // "Static" flags share the lifetime of the fiber/hook they belong to,
        // so we should bubble those up even during a bailout. All the other
        // flags have a lifetime only of a single render + commit, so we should
        // ignore them.

        subtreeFlags |= _child.subtreeFlags & StaticMask;
        subtreeFlags |= _child.flags & StaticMask; // Update the return pointer so the tree is consistent. This is a code
        // smell because it assumes the commit phase is never concurrent with
        // the render phase. Will address during refactor to alternate model.
      ...
      case SuspenseComponent:
        {
          ...
          if ((workInProgress.flags & DidCapture) !== NoFlags) {
            ...
            workInProgress.lanes = renderLanes; // Don't bubble properties in this case.
  ...
  function logCapturedError(boundary, errorInfo) {
    try {
      ...
      var error = errorInfo.value; // In production, we print the error directly.
  ...
  function throwException(root, returnFiber, sourceFiber, value, rootRenderLanes) {
    ...
    if (value !== null && typeof value === 'object' && typeof value.then === 'function') {
      ...
      var wakeable = value;
      resetSuspendedComponent(sourceFiber); // Schedule the nearest Suspense to re-render the timed out view.
  ...
  var rootWithNestedUpdates = null; // If two updates are scheduled within the same event, we should treat their
  ...
  function scheduleUpdateOnFiber(root, fiber, lane, eventTime) {
    checkForNestedUpdates(); // Mark that the root has a pending update.
  ...
  function handleError(root, thrownValue) {
    do {
      ...
      try {
        ...
        resetHooksAfterThrow(); // TODO: I found and added this missing line while investigating a
  ...
  function commitRootImpl(root, recoverableErrors, transitions, renderPriorityLevel) {
    ...
    if (subtreeHasEffects || rootHasEffect) {
      ...
      var shouldFireAfterActiveInstanceBlur = commitBeforeMutationEffects(root, finishedWork); // The next phase is the mutation phase, where we mutate the host tree.
      ...
      commitLayoutEffects(finishedWork, root, lanes); // Tell Scheduler to yield at the end of the frame, so the browser has an
  ...
  ReactDOMHydrationRoot.prototype.render = render;
  ReactDOMRoot.prototype.render = render;
  ...
  function render$1(element, container, callback) {
    if (!isValidContainerLegacy(container)) {
      throw new Error('Target container is not a DOM element.');
    }
